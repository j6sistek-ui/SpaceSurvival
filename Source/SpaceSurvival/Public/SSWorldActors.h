...
{
    SmallAsteroid, MediumAsteroid, MassiveAsteroid, Wreckage,
    ElectricalStorm, GravityAnomaly, Pursuer, Flanker, Projectile, Pickup, Event, Depot
};
...
public:
    ...
    virtual FVector GetVelocity() const override { return LinearVelocity; }
    UFUNCTION(BlueprintCallable) virtual void ReceiveWeaponHit(float Damage);
    UFUNCTION(BlueprintCallable) void Configure(ESSWorldKind InKind, float InRadius, float InDamage, int32 InWave = 1);
    UFUNCTION(BlueprintCallable) void SetLinearVelocity(FVector Value) { LinearVelocity = Value; }
    UFUNCTION(BlueprintCallable) void ApplyWorldForce(FVector Acceleration, float DeltaSeconds);
    UFUNCTION(BlueprintPure) ESSWorldKind GetKind() const { return Kind; }
    UFUNCTION(BlueprintPure) float GetBodyRadius() const { return BodyRadius; }
    UFUNCTION(BlueprintPure) virtual FString GetLabel() const;
    UFUNCTION(BlueprintPure) bool IsWeaponTarget() const;
    bool IsSolidHazard() const;
    bool IsEnemy() const;
    bool IsEnvironmentalField() const;

    UPROPERTY(EditAnywhere, BlueprintReadWrite, Category="Presentation") TObjectPtr<UStaticMeshComponent> Visual;
    UPROPERTY(VisibleAnywhere, BlueprintReadOnly, Category="Collision") TObjectPtr<USphereComponent> Collision;
    UPROPERTY(EditAnywhere, BlueprintReadWrite, Category="Tuning") float LifetimeSeconds = 65.f;
    UPROPERTY(EditAnywhere, BlueprintReadWrite, Category="Tuning") float TelegraphSeconds = 3.5f;
    UPROPERTY(EditAnywhere, BlueprintReadWrite, Category="Tuning") float GravityAcceleration = 650.f;
    UPROPERTY(EditAnywhere, BlueprintReadWrite, Category="Tuning") bool bPersistentAcrossWaves = false;
protected:
...
public:
    ...
    UFUNCTION(BlueprintPure) bool IsChargingShot() const { return ShotCharge > 0.f; }
protected:
...
public:
    ...
    virtual void ApplyWorldOffset(const FVector& InOffset, bool bWorldShift) override;
    virtual void ReceiveWeaponHit(float Damage) override {}
    virtual FString GetLabel() const override { return TEXT("WORMHOLE PASSAGE · MAINTAIN CONTROL"); }
private:
    UPROPERTY() TArray<TObjectPtr<UStaticMeshComponent>> PassageRings;
...
public:
    ...
    virtual void Tick(float DeltaSeconds) override;
    virtual void ReceiveWeaponHit(float Damage) override {}
    UFUNCTION(BlueprintPure) int32 GetPickupKind() const { return PickupKind; }
    UFUNCTION(BlueprintPure) float GetAmount() const { return Amount; }
...
UENUM(BlueprintType)
enum class ESSEncounterKind : uint8 { SalvageCache, DistressCombat, MobileDepot };
...
public:
    ...
    virtual void Tick(float DeltaSeconds) override;
    virtual void ReceiveWeaponHit(float Damage) override {}
    UFUNCTION(BlueprintCallable) bool TryAccept();
    UFUNCTION(BlueprintPure) bool IsDepot() const { return EncounterKind == ESSEncounterKind::MobileDepot; }
    UFUNCTION(BlueprintPure) TArray<int32> GetOffers() const { return Offers; }
    UFUNCTION(BlueprintPure) float GetDiscount() const { return Discount; }
    UFUNCTION(BlueprintPure) float GetInteractionRadius() const { return InteractionRadius; }
    UFUNCTION(BlueprintPure) FString GetEncounterLabel() const;
    virtual FString GetLabel() const override { return GetEncounterLabel(); }
    UFUNCTION(BlueprintPure) bool IsAccepted() const { return bAccepted; }
    UFUNCTION(BlueprintPure) bool IsResolved() const { return bResolved; }
    UFUNCTION(BlueprintPure) int32 GetObjectiveRemaining() const { return ObjectiveRemaining; }
    UFUNCTION(BlueprintPure) bool IsPlayerInRange() const;
...
public:
    ...
    UFUNCTION(BlueprintCallable) void Configure(int32 InWave, bool bInClimax);
    UFUNCTION(BlueprintCallable) void ResetEncounter();
    UFUNCTION(BlueprintCallable) void SetBreathing(bool bValue);
    UFUNCTION(BlueprintPure) float GetPressure() const { return Pressure; }
    UFUNCTION(BlueprintPure) int32 GetActiveThreatCount() const;
    UPROPERTY(EditAnywhere, BlueprintReadWrite, Category="Fairness") float MinimumReactionSeconds = 3.5f;
    UPROPERTY(EditAnywhere, BlueprintReadWrite, Category="Fairness") int32 MaximumActiveThreats = 24;
    UPROPERTY(EditAnywhere, BlueprintReadWrite, Category="Fairness") float PlayerClearanceRadius = 350.f;
    UPROPERTY(EditAnywhere, BlueprintReadWrite, Category="Tuning") float BaseBudgetPerSecond = 1.5f;
private:
    ASSShip* FindShip() const;
