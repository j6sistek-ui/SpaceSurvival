...
float ASSHUD::Paragraph(const FString &Value, float X, float Y, float Width, float Size, FLinearColor Color)
{
    ...
    GetTextSize(TEXT("Mg"), TextWidth, LineHeight, GEngine->GetMediumFont(), Size * Scale);
    ...
    for (const auto &Part : Paragraphs)
    {
        ...
        for (const auto &Word : Words)
        {
            ...
            float Height;
            GetTextSize(Candidate, TextWidth, Height, GEngine->GetMediumFont(), Size * Scale);
...
void ASSHUD::DrawHUD()
{
    ...
    if (GM->IsMenuOpen())
    {
        ...
        const float DetailHeight = Paragraph(GM->PanelDetail, X + 30 * Scale, Y + 72 * Scale, PW - 60 * Scale, .7f,
                                             FLinearColor(.68f, .75f, .85f));
        float RowY = Y + 100 * Scale + DetailHeight;
        ...
        for (int I = 0; I < GM->Entries.Num(); ++I)
        {
            ...
            DrawRect(Selected ? FLinearColor(.075f, .18f, .25f) : FLinearColor(.025f, .045f, .07f), X + 25 * Scale,
                     RowY, PW - 50 * Scale, RowH - 4 * Scale);
            Text((Selected ? TEXT(">  ") : TEXT("   ")) + Entry.Label, X + 34 * Scale, RowY + 7 * Scale, .76f,
                 Entry.Enabled ? FLinearColor(.92f, .95f, 1) : FLinearColor(.36f, .4f, .47f));
            MenuBounds.Add(
                FBox2D(FVector2D(X + 25 * Scale, RowY), FVector2D(X + PW - 25 * Scale, RowY + RowH - 4 * Scale)));
            RowY += RowH;
        }
    }
}
