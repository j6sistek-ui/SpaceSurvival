...
- Every PR, including docs-only work, must complete `.github/pull_request_template.md`. For each review group state **Updated** with files/reason, or **Reviewed unchanged** with a reason. No blank/checklist-only attestation and no needless edits to unaffected documents.
- `docs/KNOWN_ISSUES.md` is the single active issue, follow-up, priority and owner-acceptance log. Use stable IDs, an owner, next action and closure evidence. Record owner feedback there; retain partial/failed checks and repairs awaiting owner retest. Do not silently drop open items or mark them closed from a merge, code existence or automated pass.
- `docs/PROJECT_STATE.md` owns the current source/build/release distinction and local-versus-GitHub storage map. Root `PROJECT_STATE.md` is only a redirect. README links the log/state; `PLAYTEST_TOMORROW.md` redirects to the canonical cases. Never create another current status/priority list.
- The solution catalog records resources, acquisition and evaluation. Work packages/quality plans record scope and dependencies. Their historical proposals are not a competing current task queue. Preserve immutable validation receipts as history rather than copying old package numbers into current instructions.
- Review README, state, issues, solution catalog, build/run, and affected architecture/content/tuning/validation/performance/release/Phase 2 documents against the actual diff. Update `.agent/CONTINUITY.md` for meaningful state/decisions. Preserve GAME_SCOPE/IMPLEMENT authority; documentation cleanup cannot authorize scope changes.
- Every PR must tell the owner **Open / Check / Still open**: exact review destination and build identity, one to three checks/decisions, and remaining work. Keep the next action visible after answering owner questions. Owner preference/feel feedback is not a substitute for lead implementation or QA.
- Run `python Scripts/CheckPrDocumentation.py --repo .` for owner-log counts/navigation and `python Tests/TestPrDocumentation.py` when changing the gate. The Documentation review workflow also validates ready-PR declarations on body edits, new commits and ready-for-review transitions. It detects missing review records, not semantic truth; human review is still required. Do not merge a ready PR with a failing documentation check. Server-side required-status rules must be verified separately.