...
**Phase 1: PARTIAL.** The visual pass is merged and itch **0.1.16-alpha.1 / build 1979965** is ready on windows-alpha (verified September 15 UTC / September 14 local). [PRs #11–14](https://github.com/j6sistek-ui/SpaceSurvival/pulls?q=is%3Apr+is%3Amerged+11..14) are merged; PR #14's Station Workshop merge is `e44decdd`. [PR #15](https://github.com/j6sistek-ui/SpaceSurvival/pull/15) carries the owned audio/character/staff follow-up; its GitHub state is authoritative. Neither source-only follow-up changes the published game binaries. [Open work and your next review](KNOWN_ISSUES.md) owns active priorities, owner checks and closure status. This page owns the source/build/release and storage explanation.

## Source, build and release

| Layer | Last verified state | What it means |
| --- | --- | --- |
| GitHub source | PRs #11–14 merged; PR #15 owns this follow-up | Documentation foundation, visual pass, release guard and Station Workshop are in main. PR #15 adds the local owned-asset integration; its merge state and resulting main commit must be read from GitHub. Source merge and itch publication remain separate. |
