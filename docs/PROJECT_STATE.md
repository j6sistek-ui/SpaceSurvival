...
**Phase 1: PARTIAL.** Star-adjusted visual pass ready for owner review, September 14, 2026; [PR #12](https://github.com/j6sistek-ui/SpaceSurvival/pull/12) is open and unmerged, stacked on [PR #11](https://github.com/j6sistek-ui/SpaceSurvival/pull/11). [Open work and your next review](KNOWN_ISSUES.md) owns active priorities, owner checks and closure status. This page owns the source/build/release and storage explanation.

## Source, build and release

| Layer | Last verified state | What it means |
| --- | --- | --- |
| GitHub source | PRs #3, #6, #9 and #10 merged; main synchronized locally at `425e3b2` before documentation-foundation PR #11 | Foundation, refresh, catalog and merge handoff are in main. This is a dated checkpoint, not an automatically updating branch indicator. |
...
| Current visual source | [PR #12](https://github.com/j6sistek-ui/SpaceSurvival/pull/12), `codex/visual-enhancement`, packaged source `cf6296f286dba9a89583649e19f988ed09af2f16`, stacked on PR #11 at `b2304546`; later commits update documentation/receipts only | Material-only star adjustment after `4dc45ac`; C++ remains unchanged from Editor Build 12 (44.21 seconds), 49 passing Unreal tests at 23:57:10 UTC and the 30-file C++ format audit. Those checks were not rerun for the star adjustment. The 725-file fresh Git export passed at `cf6296f`. No merge or itch upload. |
...
| Published tester build | Last verified itch `0.1.15-alpha`, build `1978147`, source `602be07`; [release record](ITCH_RELEASES.md#september-14-update) | Older than local Package 4; experimental clouds disabled in that release. Later source merges did not upload it. Devlog publication remains open. |
