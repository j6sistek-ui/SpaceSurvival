...
| This documentation change | Branch `codex/documentation-review`, based on that main checkpoint | Consolidates work/status navigation and requires PR documentation review. No gameplay, imported asset, save or package change. PR state is authoritative on GitHub. |
...
For an external modeler, the recommended starting point is one private shared Drive handoff folder, separate from the live Unreal project. Keep revisions such as `Hero/v003/` together: editable source (for example Blender), agreed export, textures, preview and a short change note tied to ISS/WBS ID. Upload a new revision rather than overwriting an accepted delivery. The owner chooses access; no folder, collaborator invitation or sync automation has been created.
