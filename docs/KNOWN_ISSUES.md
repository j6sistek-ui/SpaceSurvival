...
The current visual enhancement pass is in validation. The lead is preparing a new local package and evidence before the next short controller-first review; the existing package is still identified in Project State.
...
| --- | --- | --- | --- |
| 1 | Complete the authorized purchased-asset visual enhancement pass | Lead | Native import and initial rendered checks complete; finish combat/station captures, package and verify selected cooked assets. Preserve the entry-level handling baseline. |
| 2 | Review the refreshed environment, combat cues and station | Owner; lead records and fixes findings | Use the new recorded package when available. Compare nebula depth, readable hits/deaths, visible purchased hardware and station services. Then select the largest remaining defect. |
...
The owner purchased catalog C23, Space Ship 02 Modular Pack (Havolk). Prepared derivatives supply the existing enemy roles and visible hardware for existing purchased tiers/utilities; Phase 1 still contains only the starter and scoped second ship. Future ship content stays deferred. ISS-02 owns art follow-up and ISS-14 the collaboration/storage setup; the owner confirmed the collaborator delivers model/texture/animation files only, not Unreal project edits. A private versioned Drive handoff is the proposed starting workflow; destination/access setup remains open.
...
| --- | --- | --- | --- |
| <a id="iss-01"></a>ISS-01 | Space depth, lighting, VFX and wormhole arrival remain below the visual target. The visual pass adds three blended owned nebula cubemaps, warm key light, eight ambient rock shapes, smaller dust grains, distant structures and bounded combat effects. Initial corrected sky was rendered; moving combat/station/package checks are in progress. Thin exhaust/nozzle fit and bounded parallax remain. Owner wanted arrival to feel unfamiliar (F08). [Combined record](production/COMBINED_SPACE_LOOK.md). | Needs iteration / lead | Use owner comparison to select the next visual change; verify in moving combat and both climaxes. Close with accepted Hybrid presentation/readability, not a screenshot alone. PT-03,09,12,13,18. |
...
| <a id="iss-03"></a>ISS-03 | Station exterior/interior, NPC/service animation and ambience remain unfinished. New corridor dressing, schematic screens, two idle robot staff and selected electronics are implemented; private material usage and cooked inclusion are being verified. Station4 remains the destination after candidate review; exterior retains its conservative solid collision envelope. [Environment record](ENVIRONMENT_REFRESH.md). | Needs implementation / lead | Correct material warnings and improve coherent compact service presentation; test approach, collision, walk/exit and readable services. PT-09,10,17,18. |
...
| <a id="iss-06"></a>ISS-06 | Natural combat, hit/death certainty, target identification, reward cadence and Director fairness need integrated play. F05/F06 report easy aiming and unclear kills; new bolt/muzzle/impact/explosion effects and deterministic projectile/fragment/admission repairs do not settle feel or balance. [Gameplay repairs](GAMEPLAY_QUALITY.md), [combat cues](COMBAT_CUES.md). | Needs verification/tuning / lead | Play both weapons and enemy patterns amid hazards; capture unclear deaths/unfair hits; tune from evidence while preserving manual aim and scoped rosters. PT-06,07,09,12–14,19. |
...
| <a id="iss-11"></a>ISS-11 | Distribution: clean-PC startup and real itch A-to-B update/save preservation remain unverified; the new visual build is being prepared locally, and 0.1.15 devlog is still a draft. No itch upload is authorized for this pass. [Release workflow](ITCH_RELEASES.md). | Needs release/QA follow-up / lead | Prepare a supported release receipt when publication is authorized, publish the approved devlog, then test install/update with preserved saves and version identity. Merge is not publication. |
