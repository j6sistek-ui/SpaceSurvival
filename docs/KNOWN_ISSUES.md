...
3. **Try one lasting station edit.** Open `C:/Users/j6sis/SpaceSurvival/SpaceSurvival.uproject`, then **BP_StationVisualLayout** using the [first-prop guide](STATION_EDITING.md#add-your-first-prop). Move or add one component, Compile/Save and restart Play to see it in the station. Keep the marked corridor and fixed service anchors clear; check furnishings, robot staff and service-label readability from both sides. This editing check does not close the natural station cases PT-09,10,17,18.

Start with item 1; the lead records your build, device/settings and observations against the corresponding IDs, then brings you back to the next unresolved step. You do not need to edit Markdown or remember the full checklist. Partial observations leave the corresponding acceptance case open.
...
| --- | --- | --- | --- |
| 1 | Review the new flight view, combat cues and station editing | Owner; lead records findings | Start with the flight-view check above using Package 4. Keep the entry-level handling baseline and report the largest remaining problem. |
| 2 | Follow up on the next concrete owner finding | Lead | The packaged review evidence is complete. Record the next finding, reproduce it and address the selected concrete defect. No additional feature pass is implied. |
| 3 | Retest the affected case and resume the existing loop | Lead + owner for feel | Use the relevant open case below, then continue natural five-wave/ten-wave review as appropriate. Automated tests do not close owner acceptance. |
...
| <a id="iss-11"></a>ISS-11 | Distribution: clean-PC startup and real itch A-to-B update/save preservation remain unverified. The new local Package 4 passed its actual archive/dependency audit and both packaged visual captures. The 0.1.15 devlog remains a draft, and no itch upload is authorized for this pass. [Release workflow](ITCH_RELEASES.md). | Needs release/QA follow-up / lead | Prepare a supported release receipt when publication is authorized, publish the approved devlog, then test install/update with preserved saves and version identity. Merge is not publication. |
