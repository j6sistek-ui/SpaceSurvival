...
## Current instruction: bounded packaging diagnosis; ACT-03 remains held

**Latest follow-up, September 16 06:55 UTC (ISS-11; lead):** owner authorized continuing the small pass. An unchanged diagnostic package first passed, then run `bd640abc40b94168a00955202af0487c` failed at stage 5 with `Gallery exited before requested scene was ready.` while switching to the asset map. Both maps are in the actual package index. First-failure preservation and new lifecycle/input markers are implemented; gallery behavior is unchanged. Three subsequent runs of the lifecycle-instrumented package passed the complete round trip, which does not resolve the intermittent failure. A synthetic input experiment crashed in Unreal's base controller tick, was removed, and the restored Editor rebuild passed (54.66s); it is not evidence of a player-facing crash or confirmed root cause. This now exceeds the small diagnostic pass. **Next bounded task:** reproduce with a real local-player/viewport context and these markers, distinguish input-triggered exit from load/view failure, then fix only the demonstrated cause and rerun packaged checks. The input guard occurring after gallery handling is a source-backed hypothesis, not a diagnosis. [Exact evidence and transfer packet](validation/2026-09-16-gallery-package-diagnosis.json). No release payload, merge or upload; ISS-11 remains blocked.
...
| <a id="iss-11"></a>ISS-11 | Distribution: PR17's local package builds but has an intermittent gallery exit during asset-map switching; diagnostics and exact evidence are linked at the top of this log. No proven fix or 0.1.17 payload yet. Prior 0.1.16-alpha.1 remains the published ready build1979965; release preparation filters runtime saves/config/logs. Clean-PC startup and real itch A-to-B update/save preservation remain unverified. Devlog remains a draft pending browser sign-in. [Release workflow](ITCH_RELEASES.md). | Blocked candidate / lead | Reproduce and fix only the demonstrated gallery cause, complete packaged validation and prepare the payload. Upload/merge require owner direction. Retain clean-install/update and devlog follow-ups. |
