...
**Art requirement, September 16:** diversity, style, lighting, thin nebular fog and feel are the goal, not placement efficiency. Keep a rich distant asteroid backdrop through most areas; open foregrounds should reveal recognizable distant fields/belts and structures that change scale/parallax on approach. Review whole scenes from outside and within fields. Owner supplied the alien megastructure kit (catalog A24); locally staged and metadata-inspected, not rendered or integrated yet. Lead must audition its authored assemblies, then produce different area examples and long-zone continuity. Prior visual NOT MET remains unchanged.

**Latest owner clarification, September 16:** different objects and different areas across longer zones, with sparse/dense stretches controlled by style ratios/formulas. More camera views of one scene are insufficient. Lead's next bounded operation: turn the owned inventory into three distinct area recipes and a stable procedural-cell prototype with authored focal groups, preserving open routes and existing gameplay budgets. [Proposed recipes and formula](production/COMBINED_SPACE_LOOK.md#long-zone-variety-proposed-style-rules-not-implemented) are design only; the current fixed twelve-placement shell does not implement them. Review multiple areas, transitions and longer travel with the independent agent. Zone/style changes must remain independent of wave/station cadence under GAME_SCOPE sections 24/31; future longer waves are not a current duration change.

**Independent review, September 16: NOT MET; lead agrees.** Latest capture has stronger structural framing, but large rocks block the turned aim corridor, intermediate clusters are weak, wreck surfaces are too dark and localized haze does not separate the layers. Carry these corrections into each area recipe, including moving the dominant rock pair out of the route. This item remains ACT-03, not a new issue queue.

September 16 continuation: owner supplied three Jump Space frames, prioritizing the first, and accepted the [generated concept](production/COMBINED_SPACE_LOOK.md#september-16-orbital-wreck-composition-target) as the comparison target. The editor lock cleared; owned wreck derivatives are imported, twelve major placements are authored, and multiple real Unreal captures now exist. **Target acceptance remains open.** An independent agent rejected the first render; subsequent framing, lighting and background revisions are under the same review gate. The older asteroid checkpoint below remains separate.

Acceptance gate for the area-recipe work above: **Owner-required independent acceptance gate:** a separate agent must compare the actual Unreal evidence to the accepted concept and agree with the lead that the target is met. A PARTIAL or NOT MET verdict returns concrete gaps to ACT-03 for another iteration; it cannot be overridden by a successful build or asset counts. Bind the verdict to source/content identity, capture paths and tested views. Latest valid capture: `Artifacts/EndgameSoak/b8e2f95727b6444ab64294d53664a9c7` (four 1080p views plus 84 timestamped samples). Frequent screenshot readbacks perturb timing; these do not establish smooth motion or performance. Current targets are distant scenery, not traversable obstacles. No new package or visual acceptance is claimed. [Iteration receipt](validation/2026-09-16-orbital-wreck-review.json).
...
Validation for that operation: review sustained straight travel, broad turns, revisit and sparse/dense transitions at normal speed, especially shell recycling, scale changes, near/middle separation, projectile readability and particle clutter. No further purchase is required for that review. Keep the fuzzy/flat-background complaint open: a cubemap resolution comparison is not a depth solution.

Owner direction, September 15: execute the paired ACT-00–ACT-03 flight/combat/space vertical slice before considering more purchases. Source `b6c58b889544445ca9593ab1503904dd095d1ccf` is the earlier slice checkpoint, superseded for ACT-03 by `16007ab`. It does not fix the separately captured input/station bugs, publish itch, replace the current package or close owner acceptance. ACT-04 received a compatibility and dependency audit only; the installed Wormhole Portal plugin remains disabled until a separate controlled integration pilot. Continue appending observations to the surviving ISS/RPT rows instead of creating a competing backlog. The Station Workshop remains available for owner composition work; see [Station editing](STATION_EDITING.md).
