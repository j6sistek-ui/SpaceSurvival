...
Owner direction, September 14: collect reports for now; do not start bug fixes or another broad enhancement pass. The immediate authoring question is whether the owner can arrange owned assets visually and preserve that layout for integration. The proposed editor workshop is described in [Station editing](STATION_EDITING.md#proposed-station-workshop); it has not been built.
...
| 2 | Define the visual station authoring workflow | Lead; owner chooses workflow | Recommend an Unreal editor workshop with asset browsing, placement and saved-layout integration; see ISS-09 and the station guide. Proposal only; no in-game build mode or paid plugin has been added. |
