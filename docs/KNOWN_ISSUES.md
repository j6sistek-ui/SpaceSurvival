...
**Still open:** lead must validate the compiled integration, inspect rendered motion/combat/arrival, and retain failed attempts. Physical keyboard/mouse/controller feel, listening, natural Waves 1–10, a new packaged launch and representative performance require their own evidence. The Phoenix's small native physics sphere versus larger rendered hull requires a coordinated collision-shape/query follow-up; blindly enlarging the sphere at the current pivot penetrates the pad. A rear-door boarding animation is not supplied by this reset. Phoenix paint customization remains unavailable: the three supplied materials expose no vector/color controls for the four sections. The service now declares factory finish and rejects unavailable/stale choices without changing saved classic colors; new paint material authoring remains lead-owned follow-up. Existing station art/layout and unrelated reports stay open. No merge or itch publication is authorized by this repair.
