...
**Bounded follow-up, September 16 06:34 UTC (ISS-11):** both vendor maps are present in the actual package index (`.agent/local/Package017Index.csv`). The test was overwriting its first failure with the generic return/session message; `SSWave10Soak.cpp` now logs `ENDGAME_FIXTURE_FIRST_FAILURE` with gallery stage and retains the original reason. The diagnostic package builds (201.06s), formatting and 32 structural checks pass. Identical scripted packaged replay `e0816352591a4ad18e9e2356f561e67b` succeeds at all four stages and preserves saves. This is a diagnostic-only change, not a demonstrated gallery fix. Original cause remains unconfirmed; investigating intermittent/first-launch behavior exceeds the requested small pass. Owner requested a handoff prompt if scope grows. Lead next operation: compare isolated cold/warm launches with the new first-failure log, then change only the proven cause. No release preparation or upload; acceptance remains open.

**Packaging blocker, September 16 06:22 UTC (ISS-11; lead):** owner authorized packaging PR17 with an explicit stop-and-ask-on-issues rule. Package from `ade88ee` builds, but the packaged ALIEN WORLD fixture fails return/session checks after the doorway capture; full showcase/assets/return evidence is absent. Capture `Artifacts/EndgameSoak/bedaedd587f141f5919ae793245718aa/capture.json` confirms production saves and binaries unchanged. Cause UNCONFIRMED. Packaging work is stopped awaiting owner permission to investigate/fix; no itch payload or upload. Current candidate and preserved previous archive locations are in PROJECT_STATE. Existing ACT-03 art hold remains in force.
