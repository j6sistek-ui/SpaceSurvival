...
**Phase 1 remains PARTIAL; no Phase 2 expansion is authorized.** Package 11/source691 remains the audited archive, and Package 12 is pending. Current station/sky/HUD and gameplay source passed the 21.69-second Editor build and 35 Unreal tests at 2026-09-13 14:50:03 UTC (2.892839 seconds, zero test warnings/failures/not-run cases). The owner has prioritized gameplay and may clean up the supplied hero externally; ignored HeroAlpha experiments are paused and unadopted. See [current state](PROJECT_STATE.md) for the exact changing build/package boundaries, [architecture](ARCHITECTURE.md) for ownership and [validation](VALIDATION.md) for retained history.
...
The [gameplay quality record](GAMEPLAY_QUALITY.md) and [validation receipt](validation/2026-09-13-gameplay-quality.json) bind the current repairs. Keep collision impulses distinct from continuous forces and derive contact direction from swept contact, not a crossed endpoint. New projectile strategies must clip the final sweep to range/lifetime. Collection must account for relative motion and retire before another award; fragmentation must keep bounded attempts, threat capacity and predicted reaction clearance. These rules do not establish natural fairness after later steering or gravity changes.

Menu refresh must preserve the selected action, reward commits must recheck current equipment/eligibility, and purchase previews must use actual effective stats while distinguishing capacity from repair. Contract arrival reports the actual credited delta or failure for 18 seconds; it adds no save field and must not be presented as restored history. The minor wreckage path still spends budget when its spawn produces no pieces; resolve that accounting explicitly if refactoring Director admission.
