...
## Walkthrough and local accents — September24 14:06–14:50 UTC

The owner requested multiple angles and continued refinement, then explicitly approved the planet and holographic characters. New scope is localized exterior lighting, native animated advertisements, faint rear-tower haze, freight drones and a smooth pad edge with physical recessed lamps. Approved globe/character assets and the global environment are unchanged.

Walkthrough1 captured28 raw1920x1080 views of map `4c277b73`, with no errors and an unchanged map, at `Artifacts/Outpost/Captures/20260924T140633935830Z`. FrontStatus1 subsequently saved `4360398f4234a60bf6612e05b45c3b6353285a69db4b9c9684dc1ad9afb4041f`: one supported Engineering task fixture, two upper-facade washes, and three Operations displays moved above their blocking computer banks. Its six raw views and author-time placement audit passed. These earlier views remain historical.

MaterialCoverage1 was an **unsaved diagnostic**, covering1,212 components/4,867 material slots with the engine's neutral material, then restoring references without saving assets or map. Its white Operations image is not the adopted scene or lighting. The test showed that neutral substitution did not remove all fragmented surface appearance; it does not establish a precise cause. The current gallery contains only actual scene materials.

Accents1 failed before map save because in-place addition to an Unreal Python array returnedNone. Accents2 failed before map save because positional `Rotator` arguments mapped a recipe yaw to pitch. Installed UE5.8's native make function uses Roll/Pitch/Yaw; the correction uses explicit keywords, verifies rotation and retains the0.5cm bounds tolerance. These are retained failed Python attempts despite their exit0 processes.

Accents3 completed with exit0 and saved `2f428e42c8ed8accf80a4d2f26ec14b494d2140e481d90cb2886455c2a485da5`. It adds eight complete native sign assemblies, eight mounted pad floods/four market lamps, three rear local fog volumes and three freight carriers. Broad old pad light pools are retained hidden. Original Blender-authored256-segment perimeter meshes contain10,944 and9,792 triangles; native import verifies units, bounds and four material slots. Three pads retain their physical floor collision, with720 inner-edge coverage samples each and six old halos retained hidden. The six-view capture has no errors, zero failed/deferred placement probes and unchanged map. Raw review rejected the initial pale perimeter material against the dark deck.

PadFinish1 completed with **exit0**, saving **`53c644be749c4d1bf413fa8794f69dfbc557ee4f7967632997bcb78b9ef8df28`**. Only the three trim metal slots now inherit the actual adjacent dark painted-aluminium deck finish through a private child;269 player-deck references agree, native source hash is unchanged, and inherited tint/textures/roughness are preserved. The visitor floods aim atZ370cm, matching their approximatelyZ369cm hull centers, without increasing brightness. Geometry, ground collision and approved globe/holograms remain unchanged.

Final raw gallery: **`Artifacts/Outpost/Captures/20260924T144735522512Z/index.html`**. Its31 original1920x1080 PNGs cover every area and add two pad close-ups plus the cargo skyline. Capture took144.094seconds; manifest reports zero errors, `PASS_AUTHOR_TIME_ONLY` with zero failures/deferred checks, unchanged saved map and `CAPTURED_NOT_ACCEPTED`. The offline gallery validates each PNG hash and size without changing images. Independent image review confirms continuous circular charcoal edges and narrower segmented lenses; the dark housing bevels remain subtle, retained white deck inserts are still prominent, and several room displays remain dark. This is a specific visual improvement, not whole-scene AAA acceptance.

CargoPlay1 records **14/14 scoped checks** over10.012seconds/94 actual PIE samples on preceding map2f428e42. Three carriers move20.77/16.91/22.35metres. All14 children per carrier follow the bobbing/turning anchor with zero recorded relative-transform or world-tracking error. PIE ends; map and the empty save directory remain unchanged. However, the native process subsequently exits **-1073741819 /0xC0000005**, after `LogExit: Exiting`. The log has no fatal/ensure/traceback/stack identifying the cause. Its shutdown order matches the prior play validator. Retain the runtime observations and separate unresolved teardown failure; this is not a clean process pass, populated-save check, performance test or complete gameplay validation. No speculative retry or source workaround was applied. PadFinish1 only changes material/lighting, so cargo source and placement are unchanged after that motion sample.

Python compilation,38 source structural checks and documentation navigation checks pass for this follow-up. Runtime C++ is unchanged; EditorBuild5 and the three previously pinned Outpost tests remain the compile/native evidence. No full suite, package, upload or merge was performed.

## Normal illumination and large graphic walls — September24 19:25–20:00 UTC

Owner rejects the earlier dark interior/street baseline and clarifies that hologram walls mean large blue screen-like graphic panels, not only character projections. The cached Nwiro lighting/material skills were read; no live Nwiro tool was available in this session, so native offscreen Unreal Python performed the authoring and readback. Source exposure and the approved planet/character materials were preserved.

Read-only `UsableLightingInspect1` exits0 and preserves source map53c644be. It finds active1,100-lumen ceiling points,275-lumen mounted promenade lights, and the existing fixture-lens materials. Native geometry confirms ceiling strips occupy Z426–434 while the visible ceiling underside is Z397.568: the strips are buried, although the point emitters at Z340 are already below the ceiling. Installed native material parameter and opacity-graph inspection precedes changes; originals remain hash-guarded. The exposure-range CVar is0, so saved min/max8 use the ordinary luminance range, not EV100. No global exposure adjustment is made.

`OutpostUsableLighting.py` places six strip tops flush with that measured underside and emits downward from Z388.568. Six original ceiling points remain hidden; six broad area sources replace them. Final flux is two9,000lm sources in Engineering, two7,500 in Lounge and two6,500 in Operations.57 existing local lights receive bounded brighter values/ranges, including four4,200lm promenade downlights.35 existing native lamp lens slots receive separate bright neutral/cool material children. Archive capsule lenses/characters, shared environment and physical collision are unchanged. Existing native fixture meshes support the light positions; no new floor fixtures obstruct routes.

`OutpostHologramWalls.py` turns six existing window bays into large graphic panes and mounts two complete4x2m arrival billboards, adding eight native decorative parts. Exact pane/frame bounds and structural collision are guarded. Native Robot/Keos image masks, UVs and animation remain; common emission gain8 is source-relative. Wall-only private children tint the six emissive color controls cyan-blue while preserving black animation states and alpha. Billboards keep native colors. The first DigitalPanel material choice appeared nearly empty, despite successful parameter checks; the same pane mesh visibly rendered Robot/Keos, so only the unsuitable graphic selection changed. A precise DigitalPanel shader root cause was not established.

| Batch | Saved map SHA256 | Native evidence and outcome |
| --- | --- | --- |
| UsableLighting1 | `e4becb45e10772858f640b40cb42bce55f07a68a24ba3873c4ca88415c779f6e` | `usablelighting-pass-20260924T194025Z.json`; nine views in `Captures/20260924T194040019473Z`; exit0. Rooms/street clearly brighter, but floor hotspots too strong and DigitalPanel walls nearly empty. Revised rather than accepted. |
| UsableLighting2 | `5ba88e46333bea0a5d686bb12544a514f59c5f83bee3b7055104a249118cf233` | `usablelighting-pass-20260924T194929Z.json`; nine views in `Captures/20260924T194942732929Z`. Lower general flux and rendered-good graphics; bounds/capture checks pass. Process subsequently exits-1073741819/0xC0000005 after LogExit, with no cause established. Retained as mixed evidence, not a clean run. |
| UsableLighting3 | `a57659ae6d9d8585cc8964c0d956ff39b7c9f69d6267d356211d22caf659eff7` | `usablelighting-pass-20260924T195644Z.json`; nine raw1920x1080 views in `Captures/20260924T195659913176Z`; exit0, zero capture/failed/deferred placement checks, saved map unchanged during capture. Large interior panes now visibly blue; native billboard colors retained. |

These three bounded adjustment/capture cycles are separate from runtime verification. Independent visual review confirms the stronger room/street illumination; very pale floor patches, basic strip presentation and fragmented/noisy workstation/market surfaces remain quality limits. No AAA acceptance claim is made.

`CaptureOutpostLightingPlay.py` uses the real possessed `SSWalker` camera/boom and ordinary `Shot`, not HighResShot. Actual offscreen viewport resolution is1014x344; this is not a packaged or high-resolution performance test. Runtime readback records requested/actual eye position, unchanged camera PP, native exposure-range CVar, nearby light visibility/intensity and actual show-flag enumeration. Lighting, DirectLighting, DeferredLighting, SkyLighting and PostProcessing are enabled.

- **LightingPlay1:** fails before images because private `SSWalker.camera` is not an exposed Python property. Exit0 after recorded failure, not a pass. The script now resolves and verifies named CameraComponent/SpringArmComponent instances.
- **LightingPlay2:** four ordinary viewport images, zero receipt errors and exit0 on map5ba88e46. Map/save hashes unchanged, PIE stopped. The staged Engineering pose placed the pawn too near the workstation, making that image unsuitable for review; its requested pose was corrected to the clear entrance aisle.
- **LightingPlay3:** final map a57659ae, receipt `Artifacts/Outpost/LightingPlay/20260924T195922093903Z/runtime.json`, four ordinary viewport images, zero errors, exit0, map/save preservation and stopped-PIE guards true. Camera PP unchanged in every view. Engineering actual eye `(4200,-2401.154,194.182)` is clear of the workstation; all four final images were inspected. The two-second post-PIE grace is a capture cleanup policy, not proof that the earlier intermittent teardown failure is fixed.

Both final galleries are built with hash verification and unmodified PNGs. Python compile checks,38 source structural checks and diff whitespace checks pass. Runtime C++/DLL remain EditorBuild5; no unrelated gameplay suite, new package, installation, publication or merge. Source/documentation delivery is through draft PR64. Later Blender export of this outpost remains unprepared; the existing portable0.4.1 ZIP and older BuildingSandbox.blend are not a round-trip export of this scene.

## Earlier source and draft-handoff checks
