...
SpaceSurvival Phase 1 uses only the local persistence portions below. Networking, replication and RPC sections are out of scope. Preserve the existing durable save backend and suspension-consumption guarantees when evaluating latency improvements.
...
- Every state field must be classified as one of:
  - Ephemeral local runtime state (neither persisted nor replicated)
...
- Select a write path based on latency and required durability, ordering and failure behavior. Inspect the existing backend and thread restrictions before replacing it with a slot API.
...
- Verify current Engine APIs and the existing project's serialization guarantees; a custom transactional backend is not automatically a shortcut to replace.
