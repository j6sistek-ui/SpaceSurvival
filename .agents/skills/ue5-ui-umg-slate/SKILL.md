...
# Quick Start
- Identify the existing UI layer: this project uses Canvas with Slate font measurement. Use UMG, Slate or a hybrid bridge only for a justified authoring need.
...
- Every UI task must define:
  - UI layer ownership (existing Canvas, UMG, Slate, or hybrid bridge)
...
- Preserve the existing Canvas implementation for bounded fixes; consider UMG for a panel when it improves authoring or usability enough to justify migration.
