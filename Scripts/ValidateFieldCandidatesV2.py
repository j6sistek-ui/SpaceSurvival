...
def main():
    ...
    (folder/"FieldCandidatesV2Persisted.json").write_text(json.dumps(result,indent=2)+"\n",encoding="utf-8")
    if result["errors"]:
        raise RuntimeError("Field candidate validation failed; inspect FieldCandidatesV2Persisted.json")
    u.log("FIELD_CANDIDATES_V2_PERSISTED_OK; rendered appearance still unverified")
