...
spec = importlib.util.spec_from_file_location("ss_field_author",ROOT/"Scripts/AuthorFieldCandidates.py")
...
def main():
    ...
    try:
        ...
        for item in report["meshes"]:
            ...
            assert edit.get_material_property_input_node(material,u.MaterialProperty.MP_EMISSIVE_COLOR) == custom[0]
            inputs = edit.get_inputs_for_material_expression(material,custom[0])
            assert len(inputs) == 5 and all(inputs),"Unconnected field shader input"
            ...
            assert len(expressions) == 6,"Unexpected texture or extra material pass dependency"
            ...
            result["meshes"].append({"path":mesh.get_path_name(),"triangles":mesh.get_num_triangles(0),
                ...
                "simple_collision_count":0,"collision_profile":"NoCollision","no_opacity_WPO_refraction":True,
                "material_statistics":stats,"statistics_limit":"Editor compiled shader estimates; not measured frame time",
                **built_geometry(mesh)})
        assert author.protected_snapshot() == protected,"Validation changed unrelated content"
    ...
    (folder/"FieldCandidatesPersisted.json").write_text(json.dumps(result,indent=2)+"\n",encoding="utf-8")
    if result["errors"]:
        raise RuntimeError("Field candidate validation failed; inspect FieldCandidatesPersisted.json")
    u.log("FIELD_CANDIDATES_PERSISTED_OK; rendered appearance still unverified")
