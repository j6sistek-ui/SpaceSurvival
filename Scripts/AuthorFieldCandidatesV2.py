...
SOURCE = ROOT / "ContentSource/FieldCandidates/V2"
BASE = "/Game/SpaceSurvival"
VERSION = "FieldCandidate2"
KINDS = {
    "SM_ElectricalFieldCandidateV2": ("M_ElectricalFieldCandidateV2", "ElectricalField.hlsl"),
    "SM_GravityFieldCandidateV2": ("M_GravityFieldCandidateV2", "GravityField.hlsl"),
}
...
def create_material(item,key):
    ...
    custom.set_editor_property("description","Field V2: clock-fed additive amplitude; thin geometry and view-angle edge softness")
...
def main():
    ...
    (folder/"FieldCandidatesV2Import.json").write_text(json.dumps(result,indent=2)+"\n",encoding="utf-8")
    if result["errors"]:
        raise RuntimeError("Field candidate import failed; inspect FieldCandidatesV2Import.json")
    u.log("FIELD_CANDIDATES_V2_IMPORT_OK; runtime adoption remains separate")
