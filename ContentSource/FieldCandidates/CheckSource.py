...
def main():
    ...
    for item in report["meshes"]:
        ...
        assert min(math.sqrt(sum(x*x for x in v)) for v in boundary)>99.1,"Boundary enters the effect interior"
    ...
    print("FIELD_SOURCE_CHECK_PASS",json.dumps({"meshes":records,"protected_hashes":len(report["protected_sha256"]),
          "limits":"Actual OBJ geometry/UV/normal/provenance checks only; Unreal import/rendering remains unrun."}))
