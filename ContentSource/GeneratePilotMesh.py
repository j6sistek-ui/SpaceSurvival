"""Pilot-only tail skin repair. The supplied GLB and walking mesh stay unchanged.
...
def render_review(mesh, rig, selected, mask):
    ...
    for name, location in (("Rear", (1.3, 2.2, .8)), ("Front", (1.3, -2.2, .8))):
        ...
        scene.render.filepath = str(ROOT / "ContentSource" / ("PilotTail" + ("Mask" if mask else "Proposal") + name + ".png"))
...
def main():
    ...
    for seed in bm.verts:
        ...
        if coords[:, 1].min() > -.04 and coords[:, 1].max() > .10 and coords[:, 2].max() > -.10:
    ...
    record = {"status": "DERIVED_MESH_REQUIRES_UNREAL_FIT_REVIEW", "source_glb_sha256": EXPECTED,
              "derivative_sha256": hashlib.sha256(output.read_bytes()).hexdigest(), "vertices": len(positions),
              "tail_vertices_changed": len(indices), "tail_max_vertex_drift_cm_17_samples": round(max_tail_drift_cm, 8), "mask_blender_m": {"whole_connected_fur_islands": True, "min_y_greater_than": -.04, "max_y_greater_than": .10, "max_z_greater_than": -.10},
              ...
              "notes": "Pilot-only derivative, source/walking mesh unchanged. Tail curl copied rigidly from original rest shape. Only selected tail position/normal/weights/joints changed; no texture, UV, triangle or material change. Intended for A_Pilot only."}
    (OUT / "PilotMesh.json").write_text(json.dumps(record, indent=2) + "\n", encoding="utf-8")
    assert hashlib.sha256(SOURCE.read_bytes()).hexdigest() == EXPECTED
