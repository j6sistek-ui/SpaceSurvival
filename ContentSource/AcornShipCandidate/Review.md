...
Unreal preserves the recorded linear base colors, metallic values and roughness. Six opaque surfaces add only +/-0.008 roughness modulation from one level of local-position 3D texture noise. This remains attached to the ship during motion/rebasing. Cyan emission is 3.5. Glass uses its own two-sided translucent surface material at opacity 0.18, without screen-space refraction; it is an Unreal readability approximation of the Blender transmission shader, not an exact match. Blender procedural micro-bump is not transferred.
